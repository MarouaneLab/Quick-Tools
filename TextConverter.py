...
class TextCaseConverter:
    ...
    GRADIENT_STYLES = {
        ...
        'Analogous': ['#FFB347', '#FFD700', '#ADFF2F', '#7FFF00', '#00FF7F', '#00FA9A', '#00BFFF', '#1E90FF', '#6495ED', '#8A2BE2', '#FF69B4', '#FF1493'],
        'Complementary': ['#FF6F61', '#6B5B95', '#FF6F61', '#88B04B', '#6B5B95', '#FF6F61', '#00BFFF', '#FFD700', '#00FF7F', '#FF1493'],
        'Triadic': ['#FF6F61', '#6B5B95', '#88B04B', '#FF6F61', '#6B5B95', '#88B04B', '#00BFFF', '#FFD700', '#FF69B4'],
        'Tetradic': ['#FF6F61', '#6B5B95', '#FFD700', '#88B04B', '#FF6F61', '#6B5B95', '#FFD700', '#88B04B', '#00BFFF', '#FF69B4', '#00FF7F', '#FF1493'],
        # --- Original palettes ---
        'Warm':    ['#D98236', '#E8A34C', '#E6C87D', '#F6B48F', '#F7C873', '#F9E79F', '#FAD7A0', '#F5CBA7', '#FFB347', '#FFD700'],
        'Cool':    ['#36A2D9', '#4CC3E8', '#7DDDE6', '#8FCBF6', '#73C6F7', '#9FE7F9', '#A0D7FA', '#A7C5F5', '#00BFFF', '#1E90FF'],
        'Neutral': ['#B0B0B0', '#D3D3D3', '#E0E0E0', '#C8C8C8', '#B8B8B8', '#E8E8E8', '#CCCCCC', '#F0F0F0', '#A9A9A9', '#C0C0C0'],
        'Vibrant': ['#FF5E5B', '#D72660', '#3F88C5', '#F49D37', '#140F2D', '#16DB93', '#F5CB5C', '#247BA0', '#FF69B4', '#FFD700'],
        'Muted':   ['#A39887', '#B7A99A', '#C9B6A7', '#A3A380', '#B2B09B', '#B5B5B5', '#A9A9A9', '#C0C0C0', '#B0B0B0', '#D3D3D3'],
        'Pastel':  ['#FFD1DC', '#B5EAD7', '#C7CEEA', '#FFDAC1', '#E2F0CB', '#FFB7B2', '#B5B9FF', '#FFFFB5', '#FBC2EB', '#A1C4FD'],
    }
    ...
    def _generate_gradient_random(self):
        ...
        for attempt in range(max_attempts):
            if style == 'Analogous':
                ...
                colors = [hsv_to_hex((h + (i+start)*step) % 1.0, min(max(s + random.uniform(-0.08,0.08),0.5),1), min(max(v + random.uniform(-0.08,0.08),0.5),1)) for i in range(n)]
                ...
                for i in range(n):
                    if i % 2 == 0:
                        nh = (h + random.uniform(-0.04,0.04)) % 1.0
                        ns = min(max(s + random.uniform(-0.12,0.12),0.5),1)
                        nv = min(max(v + random.uniform(-0.12,0.12),0.5),1)
                        colors.append(hsv_to_hex(nh, ns, nv))
                    else:
                        nh = (comp_h + random.uniform(-0.04,0.04)) % 1.0
                        ns = min(max(s + random.uniform(-0.12,0.12),0.5),1)
                        nv = min(max(v + random.uniform(-0.12,0.12),0.5),1)
                        colors.append(hsv_to_hex(nh, ns, nv))
            elif style == 'Triadic':
                base_hex = random.choice(palette)
                h, s, v = hex_to_hsv(base_hex)
                triad_hues = [(h + i/3) % 1.0 for i in range(3)]
                colors = []
                for i in range(n):
                    th = triad_hues[i % 3]
                    nh = (th + random.uniform(-0.04,0.04)) % 1.0
                    ns = min(max(s + random.uniform(-0.10,0.10),0.5),1)
                    nv = min(max(v + random.uniform(-0.10,0.10),0.5),1)
                    colors.append(hsv_to_hex(nh, ns, nv))
            elif style == 'Tetradic':
                base_hex = random.choice(palette)
                h, s, v = hex_to_hsv(base_hex)
                tetrad_hues = [(h + i*0.25) % 1.0 for i in range(4)]
                colors = []
                for i in range(n):
                    th = tetrad_hues[i % 4]
                    nh = (th + random.uniform(-0.03,0.03)) % 1.0
                    ns = min(max(s + random.uniform(-0.10,0.10),0.5),1)
                    nv = min(max(v + random.uniform(-0.10,0.10),0.5),1)
                    colors.append(hsv_to_hex(nh, ns, nv))
            else:
                palette_hsv = [hex_to_hsv(c) for c in palette]
                sorted_palette = [x for _,x in sorted(zip(palette_hsv, palette), key=lambda t: t[0][0])]
                if n <= len(sorted_palette):
                    start = random.randint(0, len(sorted_palette)-1)
                    indices = [(start + i) % len(sorted_palette) for i in range(n)]
                    colors = [sorted_palette[i] for i in indices]
                else:
                    interp_colors = []
                    for i in range(n):
                        pos = i * (len(sorted_palette)-1) / (n-1) if n > 1 else 0
                        idx = int(pos)
                        frac = pos - idx
                        if idx >= len(sorted_palette)-1:
                            h, s, v = hex_to_hsv(sorted_palette[-1])
                        else:
                            h1, s1, v1 = hex_to_hsv(sorted_palette[idx])
                            h2, s2, v2 = hex_to_hsv(sorted_palette[idx+1])
                            h = h1 + (h2-h1)*frac
                            s = s1 + (s2-s1)*frac
                            v = v1 + (v2-v1)*frac
                        interp_colors.append(hsv_to_hex(h%1.0, min(max(s,0),1), min(max(v,0),1)))
    ...
    def _draw_gradient_preview(self):
        ...
        if self.gradient_type_var.get() == 'Radial':
            # Fast radial gradient: draw concentric ellipses (rings) instead of per-pixel
            cx, cy = w / 2, h / 2
            rx, ry = w / 2, h / 2
            steps = min(w, h) // 2  # number of rings
            for i in range(steps, 0, -1):
                frac = i / steps
                color = self._interpolate_gradient(frac)
                # Draw ellipse for this ring
                x0 = cx - rx * frac
                y0 = cy - ry * frac
                x1 = cx + rx * frac
                y1 = cy + ry * frac
                c.create_oval(x0, y0, x1, y1, outline=color, fill=color)
        else:
